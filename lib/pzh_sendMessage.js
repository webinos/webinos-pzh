                ...
                if (this.isConnected(address)) {
                    if (PzhObject.checkConnectedPzh(address)) {// If it is connected to pzh it will land here
                        try {
                            ...
                            logger.log ("send to pzh - " + address + "  - Msg Len: "+jsonString.length);// + " message " + jsonString);
                        ...
                        try {
                            ...
                            logger.log ("send to pzp - " + address + " - Msg Len: "+jsonString.length);// + " message " + jsonString);
                   ...
                   if (PzhObject.getExternalCertificate(address)){// PZH
                       PzhObject.externalConnection[message.from] = address;
                       PzhObject.connectOtherPZH(address, PzhObject.setConnParam());
                       var message_ = message, address_ = address;
                       PzhObject.on("EXTERNAL_HUB_CONNECTED", function(){
                          PzhObject.sendMessage(message_, address_);
                       ...
                       message_ = message, address_ = address;
                       PzhObject.on("PZP_CONNECTED", function(){
