...
var Provider = function (hostname, friendlyName) {
        ...
        try{
            for (myKey in config.trustedList.pzh) {
                if (config.trustedList.pzh.hasOwnProperty(myKey)) {
                    ...
                    var details=  pzhs[myKey].addLoadPzh(email, myKey, null);
                    if (details) {
                        server.addContext(details.uri, details.cert);
                        logger.log("started zone hub " + details.uri);
                        pzhs[details.uri].pzh_pzh.connect_ConnectedPzh(details.cert);
                    } else {
            ...
            logger.error("failed starting up PZHs in the farm ");
        }
    }
    ...
    function loadSession(callback) {
        ...
        config.cert = new certificateHandler();

        config.cert.initializeCertificateHandler(config.metaData);
