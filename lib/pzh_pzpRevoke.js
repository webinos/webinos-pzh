...
var Pzh_PzpRevoke = function () {
    ...
    this.revokeCert = function (pzpId, refreshCert) {
        var pzpCert = PzhObjget.getSignedCert(), crl;
        if(PzhObject.revokeClientCert(pzpId)){
            logger.log ("revocation success! " + pzpId + " should not be able to connect anymore ");

            refreshCert (PzhObject.getSesionId(), PzhObject.setConnParam());
            return ({cmd:"revokePzp", to:PzhObject.getSessionId(), payload:"success"});
        } else {
            return ({cmd:"revokePzp", to:PzhObject.getSessionId(), payload:"failed"});
        }
