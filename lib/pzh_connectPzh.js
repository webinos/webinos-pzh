...
var Pzh_connectPzh = function () {
    ...
    this.connect_ConnectedPzh = function (options) {
        var myKey;
        for (myKey in  PzhObject.getTrustedList().pzh) {
            if (!(PzhObject.getConnectedPzh().hasOwnProperty(myKey)) && PzhObject.getSessionId() !== myKey ) {
                PzhObject.connectOtherPZH(myKey, options);
            }
        }
    };
    ...
    this.connectOtherPZH = function (to, options) {
        try {
            var pzhDetails = PzhObject.getExternalPzh(to);
            ...
            connPzh = require("tls").connect (connDetails, function () {
                ...
                if (connPzh.authorized) {
                    PzhObject.handlePzhAuthorization(to, connPzh);
                } else {
