...
function doesUserExist(user, cb) {
...
PzhAdaptor.checkUserHasPzh = function(user, res) {
    pzhTLS.send(user, {type:"checkPzh", "user":user }, responseHandler(res));
};

PzhAdaptor.addPzh = function(user, res) {
    pzhTLS.send(user, {type:"addPzh", "user":user }, responseHandler(res));
};
