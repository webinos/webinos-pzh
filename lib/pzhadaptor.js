...
PzhAdaptor.fromWebUnauth = function (userId, body, cb, err) {
    // we've received a request from the web interface over XHR.
    // translate and send to the PZH TLS server
    ...
    switch (body.type) {
        case "getCertificates":
            sendCommand(userId, "getCertificates", {}, cb, -1, err);
            break;
        default:
...
PzhAdaptor.storeExternalUserCert = function (user, externalUserNickName, externalPzh, externalCerts, cb, err) {
    sendCommand(user, "storeExternalCert", 
        {"externalUserNickName":externalUserNickName, "externalPzh":externalPzh, "externalCerts":externalCerts}, cb, -1, err);
};
...
PzhAdaptor.requestAddFriend = function(user, externalNickname, externalPzh, cb, err) {
    sendCommand(user, "requestAddFriend", {"externalNickname":externalNickname, "externalPzh" : externalPzh}, cb, -1, err);
};
