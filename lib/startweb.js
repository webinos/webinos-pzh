...
function loadWSCertificate(config, certName, certLabel) {
    ...
    if (!config.cert.internal[certName] || !config.cert.internal[certName].cert) {
        ...
        if((csr=config.generateSelfSignedCertificate(certLabel, certLabel))) {
            if((clientCert= config.generateSignedCertificate(csr))) {
...
starter.startWS = function(hostname, friendlyName, callback) {
    wUtil.webinosHostname.getHostName(hostname, function (address) {
        ...
        if(config.loadWebinosConfiguration()){
            if(loadWSCertificate(config, "webssl", "PzhSSL") && loadWSCertificate(config, "webclient", "PzhWS")){
