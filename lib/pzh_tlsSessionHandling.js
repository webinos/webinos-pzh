...
var Pzh = function () {
    ...
    PzhObject.getFriendlyName  = function(name){ return (name ? (config.trustedList.pzh[name].friendlyName || config.trustedList.pzp[name].friendlyName): config.metaData.friendlyName); };
    ...
    PzhObject.getConnectedPzh  = function(id) { return (id? pzh_state.conenctedPzh[id]: Object.keys(pzh_state.connectedPzh)); };
    PzhObject.getConnectedPzp  = function(id) { return (id? pzh_state.connectedPzp[id]: Object.keys(pzh_state.connectedPzp)); };
    PzhObject.getUntrustedList = function(id) { return (id? config.untrustedList[id]: Object.keys(config.untrustedList)); };
    ...
    PzhObject.checkUntrustedList=function(id){return config.untrustedList.hasOwnProperty(id);};
    ...
    PzhObject.setTrustedList   = function(type, name){
        if (!PzhObject.checkTrustedList(name)) {
            config.trustedList[type].name = {};
            config.storeDetails( "trustedList", config.trustedList);
        }
    };
    PzhObject.removeUntrustedList=function(name) { if(checkUntrustedList(name)) delete config.untrustedList[name];};
    PzhObject.setUntrustedList=function(name, config){
        if(!PzhObject.checkUntrustedList(name)){
            config.untrustedList[name] = config;
            config.storeDetails( "untrustedList", config.untrustedList);
        }
    };
    PzhObject.setExternalCertificate=function(name, config){
        if (!PzhObject.checkExternalCertificate(name)) {
            config.cert.external[name] = config;
