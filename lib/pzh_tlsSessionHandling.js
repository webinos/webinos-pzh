...
var Pzh = function () {
    ...
    this.addLoadPzh = function (_friendlyName, _uri, _user) {
        try {
            ...
            self.config.cert = new certificateHandler();
            self.config.cert.initializeCertificateHandler(self.config.metaData);
    ...
    this.addNewPZPCert = function (_msgRcvd, refreshCert, _callback) {
        try {
            ...
            if (parent.config.trustedList.pzp[pzpId]) {
                ...
                pzpId = pzpId + "_" + Math.round((Math.random() * 100));
                if (parent.config.trustedList.pzp[pzpId]) {
                    this.addNewPzpCert(_msgRcvd, refreshCert); // Random failed to generate something unique, regenerate id
                }
            }
