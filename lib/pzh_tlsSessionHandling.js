...
var Pzh = function () {
    ...
    PzhObject.setPzhTrustedList   = function( name){
        if (!PzhObject.checkTrustedList(name)) {
            config.trustedList.pzh[name] = {};
            config.storeDetails( "trustedList", config.trustedList);
        }
    };
    PzhObject.removeUntrustedList=function(name) { if(PzhObject.checkUntrustedList(name)) delete config.untrustedCert[name];};
