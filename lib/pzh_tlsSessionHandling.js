...
var Pzh = function (hostname) {
    ...
    PzhObject.getFriendlyName  = function(name){
        ...
        return (config.trustedList.pzh && config.trustedList.pzh[name] &&config.trustedList.pzh[name].friendlyName ||
            config.trustedList.pzp && config.trustedList.pzp[name].friendlyName);
    };
    ...
    PzhObject.getConnectedDevices = function(type){
        ...
        var pzhList = PzhObject.getConnectedPzh();
        pzhList.concat(Object.keys(pzh_state.connectedDevicesToOtherPzh.pzh));
        var pzpList = PzhObject.getConnectedPzp();
        pzpList.concat(Object.keys(pzh_state.connectedDevicesToOtherPzh.pzp));
        cDevices.pzh = pzhList;
        cDevices.pzp = pzpList;
        return (type?cDevices[type]:cDevices);
    }
    PzhObject.setConnectedPzpToOtherPzh = function(connectedDevices, id){
        var PzhList = PzhObject.getConnectedPzh();
        var PzpList = PzhObject.getConnectedPzp();
        var key;
        if (!id || id == "pzh") {
            for (key in connectedDevices.pzh) {
                if (!PzhList[key]){
                   pzh_state.connectedDevicesToOtherPzh.pzh.push(key);
                }
            }
        }
        if (!id || id == "pzp") {
            for (key in connectedDevices.pzp) {
                if (!PzpList[key]){
                    pzh_state.connectedDevicesToOtherPzh.pzp.push(key);
                }
            }
        }
    };
    ...
    PzhObject.getPzhServiceCache = function(){
        ...
        for (var i = 0; i < ownServices.length; i = i + 1){
            if (contains(remoteCache, ownServices[i]) && ownServices[i].serviceAddress &&
            isSameZone(ownServices[i].serviceAddress)) {
                remoteCache.push(ownServices[i]);
            }
        }
    ...
    PzhObject.getConnectedPzp  = function(id) {
        return (id? pzh_state.connectedPzp[id]: Object.keys(pzh_state.connectedPzp));
    };
    ...
    PzhObject.signStorePzpCertificate = function(id, friendlyName, csr) {
        ...
        if (signedCert) {
            ...
            if (config.trustedList.pzp && !config.trustedList.pzp.hasOwnProperty(id)) {// update configuration with signed certificate details ..
                config.trustedList.pzp[id] = {"friendlyName": friendlyName};
                config.storeDetails("trustedList", config.trustedList);
            }
            return true;
        } else {
        ...
        if (pzhId) {
            if (!PzhObject.getConnectedPzh().hasOwnProperty (pzhId)) {
               ...
               // PzhObject.registerServices(pzhId);
                PzhObject.synchronizationStart(pzhId); 
