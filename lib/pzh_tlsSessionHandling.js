...
var Pzh = function () {
    ...
    function createPzhCertificatesConfig(user) {
        if(self.config.createDefaultDirectories()) {
            ...
            self.config.metaData.friendlyName = self.config.userData.name;// +"_"+ self.config.userData.authenticator  ;
...
var AddPzp = function (parent) {
    ...
    this.addNewPZPCert = function (_msgRcvd, refreshCert) {
        try {
            ...
            var pzpId = parent.pzh_state.sessionId + "/D" + (_msgRcvd.message.from).substring(0, 36), msg, randId;
            console.log(pzpId);
            ...
            if (parent.config.trustedList.pzp[pzpId]) {
                ...
                if (parent.config.trustedList.pzp[pzpId]) {
                    ...
                    msg = parent.prepMsg(_msgRcvd.message.from, "pzpId_Update", _msgRcvd.message.from.substring(0, 40)+"_"+randId);
                    return msg;
                }
