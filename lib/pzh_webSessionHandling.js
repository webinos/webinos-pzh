...
// Certificate exchange
    ...
    function removePzp(conn, obj, userObj) {
        userObj.removePzp(obj.message.id, pzhFunctions.refreshPzh, function(status){
            sendMsg(conn, obj.user, { type:"removePzp", message:status });
        });
    }
    ...
    function processMsg (conn,message) {
        ...
        if (validateMessage (message)) {
            if( message.message === "WEB SERVER INIT") {
                logger.info("PZH web server started.");
                // Do nothing
            } else {
