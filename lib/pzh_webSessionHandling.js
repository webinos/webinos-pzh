...
// Certificate exchange
    ...
    function storeExternalCert (conn, obj, userObj) {
        ...
        console.log("External Cert",obj.message.externalCerts);
        ...
        if (userObj.checkExternalCertificate(name) && userObj.checkTrustedList(name)) {
            ...
            if (!userObj.checkExternalCertificate(name)) {
                addExternalCertificate(userObj, name, url.hostname, obj.message.externalCerts.serverPort,
                    obj.message.externalCerts.server, obj.message.externalCerts.crl,
                    obj.message.externalCerts.photoUrl);
    ...
    function addPzh(conn, obj, userObj) {
        ...
        sendMsg(conn, obj.user, { "type": "addPzh", "message": { "id" : id , "nickname" : nickname, photoUrl: "images/userPlaceholder.png" }});
    }
