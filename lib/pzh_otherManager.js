...
var Pzh_RPC = function (_parent) {
    ...
    this.initializeRPC = function () {
        ...
        var service = wUtil.webinosService.checkForNewWebinosModules(_parent.config.serviceCache);
        if(service !== []) {
            service.forEach(function(name){
                if (_parent.config.serviceCache.length >= 1 && _parent.config.serviceCache.indexOf(name) === -1) {
                    _parent.config.serviceCache.push(name);
                } else {
                    _parent.config.serviceCache.push(name);
                }
            });
        ...
        console.log(_parent.config.serviceCache);
