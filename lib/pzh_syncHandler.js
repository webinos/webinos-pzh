...
var Pzh_Synchronization = function (_parent) {
    ...
    PzhObject.synchronizationStartAll = function(exceptionAddr){
        if (syncInstance) {
            ...
            PzhObject.getConnectedPzp().forEach(function(address){
                if (address !== exceptionAddr && JSON.stringify(syncList[address]) != JSON.stringify(hash)) {
                    PzhObject.sendMessage(PzhObject.prepMsg(address,"syncHash" , hash), address);
                }
            });
    ...
    PzhObject.synchronizationStart = function(to){
        if (syncInstance) {
            var list = prepareSyncList();
            if(PzhObject.getConnectedPzh(to)) list =  {"trustedList":list["trustedList"], "serviceCache":list["serviceCache"], "connectedDevices": list["connectedDevices"]};
            var hash = syncInstance.getObjectHash(list);
    ...
    PzhObject.synchronization_compareHash = function(id, receivedMsg) {
        try{
            if (syncInstance){
                var list = prepareSyncList();
                if(PzhObject.getConnectedPzh(id)) list =  {"trustedList":list["trustedList"], "serviceCache":list["serviceCache"],"connectedDevices": list["connectedDevices"]};
        ...
        try {
            ...
            if(syncInstance && receivedMsg !== {}) {
                ...
                if(PzhObject.getConnectedPzh(id)) {
                    list =  {"trustedList":list["trustedList"], "serviceCache":list["serviceCache"], "connectedDevices": list["connectedDevices"]};
                }
