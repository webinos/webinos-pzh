    ...
    PzhObject.syncAllPzh = function(msgType, id, list){
        try {
            if (syncInstance) {
                ...
                if (list==="{}" || !list)  list = preparePzhSyncList();
                if (msgType === "syncHash")
                ...
                for (i = 0; i < connectedPzh.length; i = i + 1) {
                    ...
                    if (id !== address && address != PzhObject.getSessionId()) {
            ...
            if(syncInstance && receivedMsg !== {}) {
                ...
                if (receivedMsg.hasOwnProperty("serviceCache")) {
                  if (list.hasOwnProperty("serviceCache")) {
                    // Remove all services belonging to this PZP before syncing.
                    // An up-to-date list of services will be added by applyObjectContents below.
                    list["serviceCache"] = list["serviceCache"].filter(function(svc) { return svc.serviceAddress !== id; });
                  }
                }

                syncInstance.applyObjectContents(list, receivedMsg, PzhObject.getSessionId());
                ...
                PzhObject.syncAllPzh("syncHash", id);
                PzhObject.syncAllPzp("syncHash", id);
    ...
    PzhObject.syncPzh = function(id, receivedMsg) {
        try {
            ...
            if(syncInstance && receivedMsg !== {}) {
                ...
                if (receivedMsg.hasOwnProperty("serviceCache")) {
                  // Remove all services belonging to this PZH before syncing.
                  // An up-to-date list of services will be added by applyObjectContents below.
                  list["serviceCache"] = list["serviceCache"].filter(function(svc) { return svc.serviceAddress.split("/")[0] !== id; });
                }
                syncInstance.applyObjectContents(list, receivedMsg, PzhObject.getSessionId());
