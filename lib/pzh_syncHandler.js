...
var Pzh_Synchronization = function (_parent) {
    ...
    PzhObject.syncAllPzh = function(id, list){
        try {
            if (syncInstance) {
                var address, i, msg;
                ...
                for (i = 0; i < connectedPzh.length; i = i + 1) {
                    ...
                    if (id !== address) {
                        msg =  PzhObject.prepMsg(address,"syncHash", hashList);
    ...
    PzhObject.syncAllPzp = function(id, list){
        try{
            if (syncInstance) {
                var address, msg;
                var connectedPzp = PzhObject.getConnectedPzp();
                if (list!=="{}") list = prepareSyncList();
        ...
        try {
            ...
            if(syncInstance && receivedMsg !== {}) {
                ...
                if (inNeedOfPzhSync === 1){
                    PzhObject.syncAllPzh(id, newPzhList);
                } else if (inNeedOfPzpSync === 1){
                    PzhObject.syncAllPzp(id, newPzpList);
                }
    ...
    PzhObject.syncPzh = function(id, receivedMsg) {
        try {
            ...
            if(syncInstance && receivedMsg !== {}) {
                ...
                if (inNeedOfSync === 1){
                    PzhObject.syncAllPzh(id, newList);
                    PzhObject.syncAllPzp(id, list);
                }
                // No active PZP on the PZH that could use this connection, close the connection
                //PzhObject.checkWhetherToKeepConnectionAlive(id);
