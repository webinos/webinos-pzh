...
var PzhServiceHandler = function(){
    ...
    function initializeRPC() {
        ...
        ownModules = wUtil.webinosService.checkForWebinosModules(require("path").join(__dirname, "../node_modules"));
        for (var i =0 ; i < ownModules.length; i = i + 1){
            if (ownModules[i].name === 'webinos-api-test' && ownModules[i].instances.length > 0){
    ...
    this.sendFoundServices = function (validMsgObj) {
        ...
        for (var i = 0 ; i < services.length; i = i + 1){
            if(PzhObject.getConnectedPzp(services[i].serviceAddress) || PzhObject.getConnectedPzh(services[i].serviceAddress) || (services[i].serviceAddress === PzhObject.getSessionId())){
                acc.push(services[i]);
            }
        }
        ...
        logger.log ("sent " + (acc && acc.length) || 0 + " Webinos Services from this rpc handler.");
    };
