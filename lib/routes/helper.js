...
exports.getCertsFromHostDirect = function(options, successCB, errorCB) {
    var innerReq = require("https").request(options, function(innerRes) {
        ...
        innerRes.on('end', function() {
            console.log("Received cert data: \n[" + data + "]\n");
...
exports.getCertsFromHostByUrl = function(url, successcb, errorcb) {
    console.log("Getting certificates from host, url: " + url);
    var parsedUrl = require('url').parse(url);
    exports.getCertsFromHostDirect(parsedUrl, successcb, errorcb);
};


exports.getCertsFromHost = function(hostNickname, hostDomain, successcb, errorcb) {
    console.log("Getting certificates from host, nick: " + hostNickname + ", domain: " + hostDomain);
    var options = {
        host: hostDomain.split(":")[0],
        port: parseInt(hostDomain.split(":")[1]) || 443,
        path: "/certificates/" + encodeURIComponent(hostNickname),
        method: "GET"
