...
module.exports = function (app, address, port, authConfig) {
    ...
    app.get('/twitterInvite', ensureAuthenticated, ensureHasPzh, hasTwitter, function(req,res) {
        var twitterInvite = require('./twitterinvite.js');
        console.log("Configuration: " + JSON.stringify(authConfig.twitter.oauth) + ", user: " + req.user.username);
        var T = new twitterInvite(authConfig.twitter.oauth, req.user.username)
        T.getContacts(function(userList) {
            var twitter = {
                "friends" : userList
            };
            res.render('main-twitterinvite', { user:getUserPath(req.user), profile:req.user, twitter:twitter, _csrf:req.session._csrf });
        }, function(err) {
            console.log(err);
            res.redirect('/');
        });
    ...
    app.get('/twitter-invite-success', ensureAuthenticated, ensureHasPzh, hasTwitter, function(req,res) {
        res.render('main-twittersuccess',{ user:getUserPath(req.user), profile:req.user, _csrf:req.session._csrf });
    });
...
*/
    ...
    app.post('/invite-friend-twitter', ensureAuthenticated, ensureHasPzh, function(req,res) {
        req.assert('twitterId', 'No twitter ID given').notEmpty().len(2,20).isNumeric();
        if (!isValid(req,res)) return;
        if (!checkCSRF(req,res)) return;
        console.log("A twitter friend is being invited to connect to this user.  ID: " + req.body.twitterId );
    ...
    function hasTwitter(req,res,next) {
        if (req.user.provider === "twitter") {
            return next();
        } else {
            res.redirect('/');
            console.log("Twitter not enabled, so we can't do twitter invitations");
        }
    }
