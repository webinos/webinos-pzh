...
PassportConfig.createPassport = function(serverUrl, cb) {
    ...
    try {
        var fbConfig = readAuthConfig("facebook");
...
function readAuthConfig(configName) {
    var path = require('path');
    var config = require(path.join('..', 'config.json'));
    if (config.hasOwnProperty("authentication")) {     
      return config.authentication[configName];
    } else {
      return {};
    }
}
    ...
    passport.use(new FacebookStrategy(fbConfig,
      function(accessToken, refreshToken, profile, done) {
        process.nextTick(function() {
            console.log("Received a facebook profile, " + util.inspect(profile));
