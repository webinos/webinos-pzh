        ...
        try {
            ...
            wUtil.webinosMsgProcessing.readJson (PzhObject.getSessionId(), buffer, function (obj) {
                wUtil.webinosMsgProcessing.processedMsg (PzhObject, obj, function (validMsgObj) {
                    logger.log ("received message from " +validMsgObj.from + " - Msg Len"+ (JSON.stringify(validMsgObj)).length);
                        ...
                        if (validMsgObj.type === "prop") {
                            ...
                            try {
                                function checkIfPzh(id){
                                    ...
                                    return (breakup.length === 1) ? true: false;
                                ...
                                function checkIfSameZone(id){
                                    ...
                                    return (breakup && (breakup[0] === PzhObject.getSessionId()) && (typeof PzhObject.getConnectedPzp(to)!=="undefined"));
                                }
                                function checkIfOtherZonePzp(id){
                                    var breakup = id && id.split("/");
                                    return (breakup && (breakup[0] !== PzhObject.getSessionId()));
                                }

                                if (checkIfPzh(validMsgObj.to) || checkIfSameZone(validMsgObj.to) || checkIfOtherZonePzp(validMsgObj.to)) {
                                    ...
                                    if (breakup.length > 0){
                                        ...
                                        var msg = {"type"        :"JSONRPC",
                                                   ...
                                                   "payload":{"jsonrpc":"2.0",
                                                              "id":validMsgObj && validMsgObj.payload && validMsgObj.payload.id,
                                                              "error":{name: "NetworkError", "code":19 , "message":"Host is unreachable"}}
                                                   };
